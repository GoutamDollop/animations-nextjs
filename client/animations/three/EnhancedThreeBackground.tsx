  ...
  useFrame((state) => {
    if (capRef.current) {
      ...
      capRef.current.position.x +=
        Math.sin(state.clock.elapsedTime * 0.5 + position[1]) * 0.002;
    }
  });
  ...
  useFrame((state) => {
    if (globeRef.current) {
      ...
      globeRef.current.position.z +=
        Math.sin(state.clock.elapsedTime * 0.8 + position[0]) * 0.001;
    }
  });
...
function Calculator({
  ...
  useFrame((state) => {
    if (calcRef.current) {
      ...
      calcRef.current.position.y +=
        Math.sin(state.clock.elapsedTime * 1.2 + position[2]) * 0.002;
    }
  });
  ...
  useFrame((state) => {
    if (particlesRef.current) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // Floating animation
        positions[i3 + 1] +=
          Math.sin(state.clock.elapsedTime * 0.5 + positions[i3]) * 0.002;
        positions[i3] +=
          Math.sin(state.clock.elapsedTime * 0.3 + positions[i3 + 1]) * 0.001;
        positions[i3 + 2] +=
          Math.sin(state.clock.elapsedTime * 0.4 + positions[i3]) * 0.0015;
