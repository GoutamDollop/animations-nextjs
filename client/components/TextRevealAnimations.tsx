...
const splitTextIntoSpans = (element: HTMLElement, splitBy: 'chars' | 'words' | 'lines' = 'words') => {
    ...
    const initTextAnimations = () => {
      ...
      headingSelectors.forEach(selector => {
        gsap.utils.toArray(selector).forEach((heading: any) => {
          ...
          if (heading.classList.contains('no-reveal') || heading.querySelector('.word-reveal')) return;
          
          splitTextIntoSpans(heading, 'words');
          
          gsap.fromTo(heading.querySelectorAll('.word-reveal span'),
            {
              y: 100,
              opacity: 0,
              skewY: 10,
      ...
      gsap.utils.toArray('p').forEach((paragraph: any) => {
        if (paragraph.classList.contains('no-reveal') || 
            paragraph.querySelector('.word-reveal') ||
            paragraph.closest('.lightbox') ||
            paragraph.closest('.cursor-info')) return;
        
        splitTextIntoSpans(paragraph, 'words');
        
        gsap.fromTo(paragraph.querySelectorAll('.word-reveal span'),
          {
            y: 30,
            opacity: 0
          },
      ...
      gsap.utils.toArray('.section-title').forEach((title: any) => {
        ...
        gsap.fromTo(title.querySelectorAll('.word-reveal span'),
          {
            y: 50,
            opacity: 0,
            rotationY: 45,
      ...
      gsap.utils.toArray('.gradient-reveal').forEach((element: any) => {
        ...
        gsap.fromTo(element.querySelectorAll('.char-reveal'),
          {
            opacity: 0,
            scale: 0,
            rotationY: 180
...
export const TextReveal = ({ 
  ...
  type?: 'default' | 'hero' | 'section' | 'card' | 'typewriter' | 'glitch' | 'gradient' | 'flip3d' | 'bouncy';
  className?: string;
  [key: string]: any;
}) => {
  const getRevealClass = (type: string) => {
    switch (type) {
      case 'hero': return 'hero-title';
      ...
      case 'gradient': return 'gradient-reveal';
      case 'flip3d': return 'flip-3d-text';
      case 'bouncy': return 'bouncy-text';
      default: return '';
    }
  };
