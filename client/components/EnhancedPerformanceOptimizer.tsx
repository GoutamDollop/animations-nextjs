  ...
  useEffect(() => {
    ...
    const initializeOptimizations = () => {
      ...
      const setupLazyLoading = () => {
        if ('IntersectionObserver' in window) {
          ...
          images.forEach(img => {
            ...
            if (!htmlImg.dataset.src && htmlImg.src && !htmlImg.classList.contains('no-lazy')) {
              htmlImg.dataset.src = htmlImg.src;
              htmlImg.src = 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMSIgaGVpZ2h0PSIxIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxMDAlIiBoZWlnaHQ9IjEwMCUiIGZpbGw9IiNjZGNkY2QiLz48L3N2Zz4=';
            ...
            mutations.forEach(mutation => {
              mutation.addedNodes.forEach(node => {
                if (node.nodeType === Node.ELEMENT_NODE) {
                  ...
                  const newImages = element.querySelectorAll ? element.querySelectorAll('img[src]') : [];
                  newImages.forEach(img => {
                    const htmlImg = img as HTMLImageElement;
                    if (!htmlImg.dataset.src && !htmlImg.classList.contains('no-lazy')) {
                      htmlImg.dataset.src = htmlImg.src;
                      htmlImg.src = 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMSIgaGVpZ2h0PSIxIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxMDAlIiBoZWlnaHQ9IjEwMCUiIGZpbGw9IiNjZGNkY2QiLz48L3N2Zz4=';
                      htmlImg.classList.add('lazy');
                      imageObserver.observe(htmlImg);
      ...
      const optimizeMemoryUsage = () => {
        ...
        const cleanupGSAP = () => {
          gsap.globalTimeline.getChildren(true, true, false).forEach(timeline => {
      ...
      const setupPerformanceMonitoring = () => {
        ...
        const measureFPS = () => {
          ...
          if (currentTime - lastTime >= 1000) {
            metricsRef.current.fps = Math.round((frames * 1000) / (currentTime - lastTime));
            frames = 0;
