...
export default function HeroBreadcrumb() {
  ...
  const pageMapping: { [key: string]: { name: string; emoji: string; color: string } } = {
    '/': { name: 'Home', emoji: '🏠', color: 'from-blue-500 to-cyan-500' },
    '/about': { name: 'About Us', emoji: '🎓', color: 'from-blue-500 to-indigo-500' },
    '/courses': { name: 'Courses', emoji: '📚', color: 'from-orange-500 to-red-500' },
    '/teachers': { name: 'Teachers', emoji: '👩‍🏫', color: 'from-green-500 to-emerald-500' },
    '/events': { name: 'Events', emoji: '🎉', color: 'from-purple-500 to-pink-500' },
    '/gallery': { name: 'Gallery', emoji: '📷', color: 'from-pink-500 to-rose-500' },
    '/contact': { name: 'Contact', emoji: '📞', color: 'from-indigo-500 to-purple-500' },
    '/testimonials': { name: 'Testimonials', emoji: '⭐', color: 'from-emerald-500 to-teal-500' }
  };

  // Get current page info
  const currentPath = location.pathname;
  const currentPageInfo = pageMapping[currentPath] || { name: 'Page', emoji: '📄', color: 'from-gray-500 to-gray-600' };
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 md:space-x-3">
        ...
        <div className={`inline-flex items-center space-x-2 md:space-x-3 px-3 py-2 md:px-4 md:py-2.5 bg-gradient-to-r ${currentPageInfo.color} backdrop-blur-md text-white rounded-full shadow-lg border border-white/20`}>
          <Home className="h-4 w-4 md:h-5 md:w-5" />
          <span className="font-bold text-sm md:text-base">
            {currentPageInfo.name}
          </span>
          <span className="text-lg md:text-xl">
            {currentPageInfo.emoji}
          </span>
        </div>
      </div>
